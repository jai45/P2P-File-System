...
public class Server{
    ...
    public static void main(String args[]) throws FileNotFoundException,IOException{
        ...
        Scanner scnr = new Scanner(ipAddressFile);
        ...
        for(int i=0;i<replicaSendPortsList.size();i++){
            ...
            for(int j=0;j<filesAvailable.size();j++){   
                ...
                DatagramPacket DpSend =
                    new DatagramPacket(buf, buf.length, ips.get(i), replicaReceivePortsList.get(i));
...
// assist program to start the server and take the input from user
                            ...
                            if(filesAvailable.contains(fileName)){
                                ...
                                if(writePermissions.get(fileName).contains(loginUser)){
                                    ...
                                    for(int i=0;i<replicaReceivePortsList.size();i++){
                                        ...
                                        ReplicaDpSend =
                                            new DatagramPacket(replicabuf, replicabuf.length, ips.get(i), replicaReceivePortsList.get(i));
                                    ...
                                    while (true)
                                    {
                                        ...
                                        if(writeData.startsWith("Done")){
                                            ...
                                            ReplicaDpSend =
                                                new DatagramPacket(replicabuf, replicabuf.length, ips.get(i), replicaPortsList.get(i));
                                                serverSocket.send(ReplicaDpSend); 
                                            }
                                            break;
                                        }
                ...
                case "search":
                            ...
                            File file = new File(fileName);
                                if(!file.exists()){
                                    ...
                                    DatagramPacket DpSend =
                                        new DatagramPacket(buf, buf.length, ips.get(serverId), replicaReceivePortsList.get(serverId));
    ...
    public static List<String> sendRead(String fileName,int peer){
        try{
            ...
            buf = inp.getBytes();
            DatagramPacket DpSend =
                new DatagramPacket(buf, buf.length, ips.get(peer), replicaReceivePortsList.get(peer));
            replicaSendSocket.send(DpSend);
            byte[] receive = new byte[65535];                      
            DatagramPacket DpReceive = null;
            while(true){
